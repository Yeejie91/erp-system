...
export default function AccountsReceivable() {
  ...
  const handleOpenPayment = (account: AccountReceivable) => {
  ...
  const handlePayment = async () => {
    ...
    if (amount > selectedAccount.remainingAmount) {
      alert(`收款金额不能超过未收金额 ${formatCurrency(selectedAccount.remainingAmount)}`);
      return;
    }
    ...
    try {
      ...
      await paymentRecordsService.add(newPayment);
      ...
      await accountsReceivableService.update(updatedAccount);

      // 如果关联的是发票，同步更新发票状态
      if (selectedAccount.relatedType === 'invoice') {
        const invoice = await invoicesService.getById(selectedAccount.relatedId);
        if (invoice) {
      ...
      await logOperation({
        ...
        targetName: selectedAccount.customerName,
        description: `收款 ${formatCurrency(amount)} - ${selectedAccount.relatedNumber}`,
      });

      alert(`收款成功！已收 ${formatCurrency(amount)}`);
  ...
  // 统计数据
  const totalReceivable = accounts.reduce((sum, a) => sum + a.remainingAmount, 0);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">管理客户欠款和收款记录</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">应收总额</p>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">本月已收</p>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-sm text-gray-500">
            共 {filteredAccounts.length} 条应收账款
          </span>
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">应收金额</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">已收金额</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">未收金额</th>
      ...
      {showPaymentModal && selectedAccount && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h2 className="text-xl font-bold mb-4">收款</h2>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-600 mt-2">未收金额</p>
      ...
      {showDetailModal && selectedAccount && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
            <h2 className="text-xl font-bold mb-4">应收账款详情</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-600">应收金额</p>
                ...
                <div>
                  <p className="text-sm text-gray-600">未收金额</p>
              ...
              {/* 收款记录 */}
              <div>
                <h3 className="text-lg font-semibold mb-2">收款记录</h3>
                {accountPayments.length > 0 ? (
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">收款日期</th>
                        <th className="px-4 py-2 text-left text-xs font-medium text-gray-500">收款金额</th>
